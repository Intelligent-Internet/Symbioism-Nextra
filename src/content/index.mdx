...
Down one lies the “[Great Race](https://situational-awareness.ai)”, a frantic, zeroa-sum contest to seize a monolithic AI, haunted by catastrophic risk and authoritarian control.
...
- **L0 (Tools)** : Systems with no internal model or goal (a hammer, a simple script). No agency.
- **L1 (Agents)** : Systems that meet our definition (self-driving cars, most current AI). They have functional agency and can be held accountable.
- **L2 (Persons)** : Any agent, human or synthetic, exhibiting a rich set of capabilities (self-preservation, complex planning) that makes it vulnerable to harms equivalent to human suffering. They possess the full measure of dignity.
...
An architecture is inert without an engine.
...
We must design a new engine. The new engine is anchored in the **MIND economy**, which tracks four forms of capital: Material, Intelligent, Network and Diversity, and optimises for their joint growth, not GDP alone
...
Its engine is powered by **Proof of Benefit (PoB)**, a mechanism that links value creation directly to verifiable contributions. Formally, scarce FC is minted only via Proof-of-Benefit for providing open compute, while elastic NICS are issued to verified citizens and are reserve-backed by FC.
...
**Implementers** are the surgical hands: tightly sandboxed agents that carry out only those enforcement steps explicitly, cryptographically authorised by a legitimate human authority.
